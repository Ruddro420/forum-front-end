...
export const AuthProvider = ({ children }) => {
    ...
    const fetchFeaturedTags = async () => {
        //   const userId = sessionStorage.getItem("userId");
        try {
            const res = await fetch(`${import.meta.env.VITE_SERVER_API}/tags/featured`);
            // console.log("respose of me",res);
            if (!res.ok) throw new Error("Failed to fetch user");
            const data = await res.json();
    ...
    useEffect(() => {
        axios.get(`${import.meta.env.VITE_SERVER_API}/categories`).then(res => {
    ...
    return (
        <AuthContext.Provider value={{ user, loading, login, logout, fetchUser, posts, stats, recentPost, fetauredTag, fetchPost }}>
            {children}
