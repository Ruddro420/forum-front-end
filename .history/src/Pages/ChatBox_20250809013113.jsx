...
const Chatbox = () => {
  const { user } = useAuth();  // user from your auth context
  ...
  // Build currentUser from API user data
  ...
  // For now, assume adminUser is fixed
  ...
  // Initialize Ably only if currentUser exists
  useEffect(() => {
  ...
  // Load past messages once currentUser is ready
  useEffect(() => {
    if (!currentUser) return;
  ...
  if (!currentUser) return <div>Please login to chat</div>;
