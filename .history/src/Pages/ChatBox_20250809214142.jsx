...
const Chatbox = () => {
  ...
  return (
     <div className="max-w-lg mx-auto bg-white rounded-xl border border-gray-200 shadow-md overflow-hidden">
      {/* Header */}
      <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-blue-500 to-purple-600 text-white flex items-center space-x-3">
        <img
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, i) => {
            ...
            return (
              <div
                key={msg.id || i}
                className={`flex items-end ${isMe ? "justify-end" : "justify-start"} space-x-2`}
              >
                {/* Avatar */}
                {!isMe && (
                  <img
                ...
                {isMe && (
                  <img
                    src="/user-avatar.jpg"
                    alt="You"
                    className="w-8 h-8 rounded-full object-cover border order-2"
                  />
                )}

                {/* Bubble */}
                <div
                  className={`max-w-xs lg:max-w-sm px-4 py-2 rounded-2xl shadow-sm ${
                    isMe
