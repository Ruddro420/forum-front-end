...
import { Send, Smile, Paperclip, Mic, CircleUserRound, ShieldUser } from 'lucide-react';
      ...
      <div className="flex items-center justify-between p-3 bg-white border-b border-gray-200 shadow-sm">
        <div className="flex items-center space-x-3">
          <div className="relative">
            <img src={adminUser.avatar} alt="Admin" className="w-10 h-10 rounded-full object-cover border-2 border-white shadow" />
                  ...
                  <div className={`flex max-w-xs lg:max-w-md ${isMe ? 'flex-row-reverse' : ''}`}>
                    {showAvatar && (
                      <div className={`flex-shrink-0 ${isMe ? 'ml-3' : 'mr-3'}`}>
                        {isMe ? <CircleUserRound className="w-8 h-8 rounded-full object-cover border-2 border-white shadow" size={10} /> : <ShieldUser className="w-8 h-8 rounded-full object-cover border-2 border-white shadow" size={10} />}
                        {/* <img src={isMe ? currentUser.avatar : adminUser.avatar} alt={isMe ? 'You' : 'Admin'} 
