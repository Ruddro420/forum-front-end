...
const QuestionCard = ({ question }) => {
  ...
  const [voteCount, setVoteCount] = useState(question.votes?.length || 0);

  const handleVote = async (type) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_SERVER_API}/post/${type}vote/${id}`, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      if (res.ok) {
        ...
        console.error("Voting failed:", res.statusText);
      }
    } catch (err) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200">
      <div className="flex gap-4">
        ...
        <div className="flex flex-col items-center space-y-2 min-w-0">
          ...
          <span
            className={`text-lg font-medium ${voteCount > 0 ? 'text-green-600' : voteCount < 0 ? 'text-red-600' : 'text-gray-500'}`}
          >
            {voteCount}
          </span>
          <button
